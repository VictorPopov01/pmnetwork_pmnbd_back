 ...
 * @var backend\modules\pmnbd\models\blog\BlogPost $model
...
>
	...
	<div class="form-group mti-20 mbi-20">
		<label class="control-label col-sm-2">Выбрать город</label>
		<div class="col-sm-8 flex_wrap">
			<?php if ($model->isNewRecord) : ?>
				можно после сохранения
			<?php else : ?>
				<?= Select2::widget([
					'name' => 'blogPostSubdomens',
					'data' => ArrayHelper::map(Subdomen::find()->all(), 'city_id', 'name'),
					'value' => array_map(function ($subdomen) {
						return $subdomen->subdomen_id;
					}, $model->blogPostSubdomens),
					'options' => [
	...
	<?php echo $form->field($model, 'collection')->checkbox([], false) ?>
